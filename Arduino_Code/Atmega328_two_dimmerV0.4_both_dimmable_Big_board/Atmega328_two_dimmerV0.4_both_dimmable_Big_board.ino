...
#define DIMMABLE_TRIAC_1     4 //Gpio 9 

/*Dual colour LED*/ 
#define DLED_RED   A4  // Big dimmer Board
#define DLED_GREEN A5
...
#define INPIN_REGULATOR A1  // input pin (push button)
#define INPIN_REGULATOR2 A2  // input pin (push button)
...
//to add 0 so that string matches channel of triac D:10xxx
  if(dimvalue_one>=10 && dimvalue_two>=10)
  {
   Serial.println("D:"+String(dimvalue_one)+","+String(dimvalue_two)); 
  }
  else if(dimvalue_one<10 && dimvalue_two>=10) //to add 0 so that string matches channel of triac D01xxx
  {
   Serial.println("D:0"+String(dimvalue_one)+","+String(dimvalue_two));
  }
   else if(dimvalue_one>=10 &&  dimvalue_two<10) //to add 0 so that string matches channel of triac D01xxx
  {
   Serial.println("D:"+String(dimvalue_one)+",0"+String(dimvalue_two));
  }
    else //to add 0 so that string matches channel of triac D01xxx
  {
   Serial.println("D:0"+String(dimvalue_one)+",0"+String(dimvalue_two)); 
  }
